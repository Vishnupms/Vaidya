...
router.post('/update-profile',authMiddleware,async(req,res)=>{
  try {
      ...
      if(!user){
          ...
          await user.save()
          res.status(200).send({message:"update profile successfully",success:true,user})
      }
...
router.post('/remove-profile',authMiddleware,async(req,res)=>{
  try { 
      ...
      const user=await User.findOneAndUpdate({_id:req.body.userId},{$set:{image:'https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_1280.png'}},{new:true})

      if(!user ){
          return res.status(200).send({message:"Userr does not exist",success:false})
      }else{
          res.status(200).send({message:"removed profile photo successfully",success:true,user})
      }
